...
const neighborhoodShipment = [
  ...
  {
    neighborhood: "Lauro Passos",
    shipment: 180,
  },
  {
    neighborhood: "Chapadinha",
    shipment: 150,
  },
];
...
function addProductToCart(event) {
  ...
  const productName = productCard.querySelector(".info-product h1").textContent;
...
function searchCep() {
  ...
  fetch(`https://viacep.com.br/ws/${typedCep}/json/`)
    ...
    .catch((error) => {
      ...
      alert(
        "Não foi possível buscar o CEP. Verifique se o CEP está correto ou tente novamente mais tarde."
      );
    });
}
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (sectionProductsMobile && savedProductsArray) {
    for (const product of savedProductsArray) {
      ...
      subtotalProduct.innerHTML = `Subtotal: <span>R$ ${subtotalP.toFixed(
        2
      )}</span>`;
...
function finishOrder() {
  ...
  window.open(`https://wa.me/5575981566646?text=${textEncoded}`);
}
...
function updateInfosOrder(discount) {
  ...
  if (
    ...
    const fondedNeighborhood = neighborhoodShipment.find(
      (info) => info.neighborhood === inputNeighborhood.value
    );

    const shipmentValue = fondedNeighborhood
      ? fondedNeighborhood.shipment
      : 150;
...
function addCoupon() {
  ...
  const validCoupon = availableCoupons.find(
    (coupon) => coupon.value === inputCoupon.value
  );
